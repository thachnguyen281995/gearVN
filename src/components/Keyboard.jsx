...
import BannerKeyBoard from "../images/bannerkeyboard.png"

// Import Swiper styles

import { Link } from "react-router-dom";
const Keyboard = () => {
    const products = datakeyboard.map((product) => {
        
        return (
            
            <div key={product.id}>
                <Link to={`/keyboard/${product.id}`}>
    ...
    return (
      ...
      <div className="flex flex-col justify-center">
        <h1 className="text-3xl lg:text-4xl font-bold text-gray-800 dark:text-white">Bàn Phím</h1>
      ...
      <div className="flex justify-end md:absolute md:bottom-4 md:right-4 lg:bottom-0 lg:right-0 hover:scale-150 cursor-pointer duration-300">
        <img src={BannerKeyBoard} alt="" className="md:w-20 md:h-20 lg:w-20 lg:h-20" />
      </div>
    </div>
